...
// Leaderboard Page
var pageLeaderboard = document.querySelector("#pageLeaderboard");
var leaderboardBackBtn = document.querySelector("#leaderboardBackBtn");
...
var leaderboardPageToggle = document.getElementById("leaderboardPageToggle");
var pagesTogglesArray = [
  ...
  playDelayPageToggle,
  pauseMenuPageToggle,
  levelPassedPageToggle,
  youLostPageToggle,
  highScorePageToggle,
  aboutPageToggle,
  splashPageToggle,
  leaderboardPageToggle,
];
var pagesArray = [
  pagePlayArea,
  pageGameMenu,
...
// Thêm sự kiện cho btnLeaderBoard
toolsBox.onClickNTouchstart(btnLeaderBoard, function () {
  audioPool.playSound(buttonTap);
  toolsBox.showPage(pageLeaderboard);
  toolsBox.hidePage(pageGameMenu);
  initLeaderboard(); // Khởi động bảng xếp hạng
});

// Thêm sự kiện cho leaderboardBackBtn
toolsBox.onClickNTouchstart(leaderboardBackBtn, function () {
  audioPool.playSound(buttonTap);
  database.ref("scores").off(); // Ngắt lắng nghe
  toolsBox.showPage(pageGameMenu);
